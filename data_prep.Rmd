...
control <- trainControl(method="repeatedcv", number=10, repeats=3, savePredictions=TRUE, classProbs=TRUE,summaryFunction = mnLogLoss)
...
print(stack.glm)
stack_glm_pred <- predict(stack.glm, newdata= d_ss_tree, type='prob')
d_ss <- d_ss %>% mutate(Pred = stack_glm_pred)

# stack using random forest
set.seed(seed)
stack.rf <- caretStack(models, method="rf", metric="Accuracy", trControl=stackControl)
print(stack.rf)
stack_predictions <- predict(stack.rf, newdata= d_ss_tree, type='prob')
d_ss <- d_ss %>% mutate(Pred = stack_predictions)
