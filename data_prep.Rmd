...
model <- glm(team1win ~ 
             ...
             data = train_glm, family = binomial)
...
predict <- data.frame(Pred = predict(model, newdata = d_ss_glm, type = 'response'))
...
train_nnet <- train[,-(1:4)]
col_order <- colnames(train_nnet)
train_nnet_scaled <- as.data.frame(scale(train_nnet[-1]))
train_nnet_scaled <- cbind(train_nnet$team1win,train_nnet_scaled)
...
nn <- neuralnet(a, data=train_nnet_scaled, hidden=c(1,1), linear.output=FALSE, threshold=0.01)
nn$result.matrix
plot(nn)
...
d_ss_nnet <- d_ss[,-1][,-(2:4)]
colnames(d_ss_nnet)[1] <- "team1win"
d_ss_nnet <- d_ss_nnet[, col_order]
d_ss_nnet_scaled <- scale(d_ss_nnet)

#predict <- data.frame(Pred = predict (model, newdata=d_ss_nnet_scaled, type="prob"))

test_set <- subset(d_ss_nnet_scaled, select = colnames(d_ss_nnet_scaled)[-1])
nn.results <- compute(nn, test_set)
...
#Tree prediction
predict(DecisionTree,d_ss_nnet)[,1]
d_ss <- d_ss %>% mutate(Pred = predict(DecisionTree,d_ss_nnet)[,1])
```
...
d_ss_tree <- d_ss_nnet
d_ss_tree$team1win <- NULL
d_ss_tree$team1win <- sample(c(0, 1), nrow(d_ss_nnet), replace=TRUE)
d_ss_tree <- d_ss_tree[, col_order]
d_ss_tree$team1win<- factor(d_ss_tree$team1win, labels=c(0,1))

random_forest <- randomForest(formula = team1win ~.,
                        data=train_tree,
                        mtry=3,      # three predictor-vars selected randomly at each split
                        xtest=d_ss_tree[-1],
                        ytest=d_ss_tree$team1win,
...
f_rf_predictions <- predict(random_forest, d_ss_tree, type='prob')
d_ss <- d_ss %>% mutate(Pred = f_rf_predictions[,2])
# 
